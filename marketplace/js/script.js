(function($) { 
	...
	/* ================ one page functions. ================ */
	if($(".digits").length > 0){
		$('.digits').countdown('2020/02/10').on('update.countdown', function(event) {
...
function alartme(){
	alert("Invalid File Format Uploaded!Allowed formats are Word Documents[.DOCX, .DOC, .RTF] and PDF");
	//window.location.href = 'Register.php';
	}
	
function alartme2(){
	alert("You have registered this user!");
	//window.location.href = 'guest-register-for-event.php';
	}
	
function alartme3(){
	alert('Registration was successful! Check you mail inbox or spam for further detail.');
	//window.location.href = 'Register.php';
	}
	
function alartme4(){
	alert('You have not register! Do so before Abstract and Paper Submission.');
	window.location.href = 'guest-register-for-conference.php';
	}
	
function alartme5(){
	alert('Submission was successful! Check your mail inbox or spam for further detail.');
	}
	
function alartme6(){
	alert('The Department field is empty! Wait for it to be populated after selecting your School');
	//window.location.href = 'Register.php';
	}
	
function alartme9(){
	alert('Registration was successful! Check your mail inbox or spam for further detail.');
...
function alartme10(){
	alert('Matric Number supplied is in wrong format!\n\n It should be in this form:00/00/0000 without space(s)');
	//window.location.href = 'home.php';
	}	
	
function alartme7(){
	alert('E-mail supplied already exist');
	//window.location.href = 'home.php';
	}		
	
// Staff Login Script

	$('#login_btn').click(function(){ // Create `click` event function for login
		var username = $('#username'); // Get the username field
		var password = $('#password'); // Get the password field
		var login_result = $('.loginStatus'); // Get the login result div
		login_result.fadeIn('slow');
		login_result.css('background','#006699');
		 // Set the pre-loader can be an animation
		
		if(username.val() == ''){ // Check the username values is empty or not
			username.focus(); // focus to the filed
			login_result.css('background','#bf0000');
			login_result.css('color','#ffffff');
			login_result.html('Enter the Username');
			return false;
		}
		...
		if(username.val() != '' && password.val() != ''){ // Check the username and password values is not empty and make the ajax request
			...
			data : UrlToPass,
			cache: false,
			url  : 'processAuthentication.php',
			success: function(responseText){ // Get the result and asign to each cases
				if(responseText == 0){
					login_result.css('background','#bf0000');
					login_result.css('color','#ffffff');
					login_result.html('Authentication Failed! Wrong account detail or not approved yet.');
					return false;
				}
				else if(responseText == 1){
					
					login_result.css('background','#58983D');
					login_result.css('color','#ffffff');
					login_result.html('Authentication Successful! Redirecting');		
					window.location = 'dashboard.php';
				}
				else{
					login_result.css('background','#bf0000');
					login_result.html('Problem with sql query! Contact the Developer');					
				}
			}
			});
...
// Student Login Script
	...
	$('#stu_login_btn').click(function(){ // Create `click` event function for login
		var username = $('#stuusername'); // Get the username field
		var password = $('#stupassword'); // Get the password field
		var login_result = $('.stuloginStatus'); // Get the login result div
		login_result.fadeIn('slow');
		login_result.css('background','#006699');
		 // Set the pre-loader can be an animation
		...
		if(username.val() != '' && password.val() != ''){ // Check the username and password values is not empty and make the ajax request
			...
			data : UrlToPass,
			cache: false,
			url  : 'processStudentAuthentication.php',
			success: function(responseText){ // Get the result and asign to each cases
				if(responseText == 0){
					login_result.css('background','#bf0000');
					login_result.css('color','#ffffff');
...
// Alumni Login Script
	...
	$('#alu_login_btn').click(function(){ // Create `click` event function for login
		var username = $('#aluusername'); // Get the username field
		var password = $('#alupassword'); // Get the password field
		var login_result = $('.aluloginStatus'); // Get the login result div
		login_result.fadeIn('slow');
		login_result.css('background','#006699');
		 // Set the pre-loader can be an animation
		...
		if(username.val() != '' && password.val() != ''){ // Check the username and password values is not empty and make the ajax request
			...
			data : UrlToPass,
			cache: false,
			url  : 'processAlumniAuthentication.php',
			success: function(responseText){ // Get the result and asign to each cases
				if(responseText == 0){
					login_result.css('background','#bf0000');
					login_result.css('color','#ffffff');
					...
					login_result.css('color','#ffffff');
					login_result.html('Authentication Successful! Redirecting');		
					window.location = 'alumnidashboard.php';
				}
				else{
					login_result.css('background','#bf0000');
					login_result.html('Problem with sql query! Contact the Developer');					
